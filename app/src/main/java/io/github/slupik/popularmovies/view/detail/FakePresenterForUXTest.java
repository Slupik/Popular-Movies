...
class FakePresenterForUXTest extends BasePresenter<DetailPresentedView> implements DetailPresenter {
    ...
    static RecyclerViewReviewList initRecycleView(Context context){
        FakePresenterForUXTest fake = new FakePresenterForUXTest(context);
        RecyclerViewReviewList mAdapter = new RecyclerViewReviewList(fake);
    ...
    private class FakeDownloading implements Runnable {
        ...
        public void run() {
            sleep(SLEEP_TIME);
            List<Review> completedList = new ArrayList<>();
